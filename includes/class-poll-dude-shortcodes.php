...
class Poll_Dude_Shortcode {
	...
	public function check_voted_ip( $poll_id ) {
		...
		$get_voted_aids = $wpdb->get_col( $wpdb->prepare( "SELECT pollip_aid FROM $wpdb->pollsip WHERE pollip_qid = %d AND (pollip_ip = %s OR pollip_ip = %s)", $poll_id, $this->poll_get_ipaddress(), $this->get_ipaddress() ) . $log_expiry_sql );
		if( $get_voted_aids ) {
			return $get_voted_aids;
		}
	...
	public function display_pollvote($poll_id, $display_loading = true) { do_action('wp_polls_display_pollvote');
		global $wpdb;
	...
	public function poll_get_ipaddress() {
		return apply_filters( 'wp_polls_ipaddress', wp_hash( $this->get_ipaddress() ) );
	}
	
	public function poll_get_hostname() {
		$hostname = gethostbyaddr( $this->get_ipaddress() );
		if ( $hostname === $this->get_ipaddress() ) {
			$hostname = wp_privacy_anonymize_ip( $this->get_ipaddress() );
		}
